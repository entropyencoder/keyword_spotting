...
def create_model(fingerprint_input, model_settings, model_architecture,
  ...
  if model_architecture == 'single_fc':
    ...
    return create_ds_cnn_model(fingerprint_input, model_settings, 
                                 model_size_info, is_training)
  elif model_architecture == 'ds_cnn_selu':
    return create_ds_cnn_selu_model(fingerprint_input, model_settings,
                               model_size_info, is_training)
  elif model_architecture == 'ds_cnn_prelu':
    return create_ds_cnn_prelu_model(fingerprint_input, model_settings,
                                    model_size_info, is_training)
  elif model_architecture == 'normal_cnn':
    return create_normal_cnn_model(fingerprint_input, model_settings,
                               model_size_info, is_training)
  elif model_architecture == 'normal_cnn_dropout':
    return create_normal_cnn_dropout_model(fingerprint_input, model_settings,
                                   model_size_info, is_training)
  elif model_architecture == 'normal_cnn_selu_dropout':
    return create_normal_cnn_selu_dropout_model(fingerprint_input, model_settings,
                                           model_size_info, is_training)
  elif model_architecture == 'resid_cnn_dropout':
    return create_resid_cnn_dropout_model(fingerprint_input, model_settings,
                                           model_size_info, is_training)
  elif model_architecture == 'resid_cnn':
    return create_resid_cnn_model(fingerprint_input, model_settings,
                                           model_size_info, is_training)
  elif model_architecture == 'normal_cnn_incr_dropout':
    return create_normal_cnn_incr_dropout_model(fingerprint_input, model_settings,
                                   model_size_info, is_training)
  else:
    raise Exception('model_architecture argument "' + model_architecture +
                    ...
                    ' "gru", "crnn", "ds_cnn",'+
                    ' "ds_cnn_selu", "ds_cnn_prelu", "normal_cnn", "normal_cnn_dropout", "normal_cnn_selu_dropout", "resid_cnn_dropout"')
...
def create_normal_cnn_model(fingerprint_input, model_settings, model_size_info,
                        is_training):
  """Builds a model with depthwise separable convolutional neural network
  Model definition is based on https://arxiv.org/abs/1704.04861 and
  Tensorflow implementation: https://github.com/Zehaos/MobileNet
  ...
  def normal_cnn_arg_scope(weight_decay=0):
    """Defines the default ds_cnn argument scope.
    Args:
      weight_decay: The weight decay to use for regularizing the model.
    Returns:
      ...
      with slim.arg_scope([slim.batch_norm],
        ...
        for layer_no in range(0, num_layers):
          if layer_no == 0:
            ...
            net = slim.convolution2d(net, conv_feat[layer_no], \
                                            kernel_size=[conv_kt[layer_no], conv_kf[layer_no]], \
                                            stride=[conv_st[layer_no], conv_sf[layer_no]],
                                            scope='conv_normal_' + str(layer_no))
            net = slim.batch_norm(net, scope='conv_normal_' + str(layer_no) + '/batch_norm')
          t_dim = math.ceil(t_dim / float(conv_st[layer_no]))
          f_dim = math.ceil(f_dim / float(conv_sf[layer_no]))
...
def create_ds_cnn_selu_model(fingerprint_input, model_settings, model_size_info,
                        is_training):
  """Builds a model with depthwise separable convolutional neural network
  Model definition is based on https://arxiv.org/abs/1704.04861 and
  Tensorflow implementation: https://github.com/Zehaos/MobileNet
...
def create_ds_cnn_prelu_model(fingerprint_input, model_settings, model_size_info,
                        is_training):
  """Builds a model with depthwise separable convolutional neural network
  Model definition is based on https://arxiv.org/abs/1704.04861 and
  Tensorflow implementation: https://github.com/Zehaos/MobileNet
...
def create_normal_cnn_dropout_model(fingerprint_input, model_settings, model_size_info,
                        is_training):
  """Builds a model with depthwise separable convolutional neural network
  Model definition is based on https://arxiv.org/abs/1704.04861 and
  Tensorflow implementation: https://github.com/Zehaos/MobileNet
...
def create_normal_cnn_selu_dropout_model(fingerprint_input, model_settings, model_size_info,
                        is_training):
  """Builds a model with depthwise separable convolutional neural network
  Model definition is based on https://arxiv.org/abs/1704.04861 and
  Tensorflow implementation: https://github.com/Zehaos/MobileNet
...
def create_resid_cnn_dropout_model(fingerprint_input, model_settings, model_size_info,
                        is_training):
  """Builds a model with depthwise separable convolutional neural network
  Model definition is based on https://arxiv.org/abs/1704.04861 and
  Tensorflow implementation: https://github.com/Zehaos/MobileNet
  ...
  def resid_cnn_arg_scope(weight_decay=0):
    """Defines the default ds_cnn argument scope.
    Args:
      weight_decay: The weight decay to use for regularizing the model.
    Returns:
      ...
      with slim.arg_scope([slim.batch_norm],
        ...
        for layer_no in range(0, num_layers):
          if layer_no == 0:
            ...
            net_resid = slim.convolution2d(net, conv_feat[layer_no], \
                                            kernel_size=[conv_kt[layer_no], conv_kf[layer_no]], \
                                            stride=[conv_st[layer_no], conv_sf[layer_no]],
                                            scope='conv_normal_' + str(layer_no))
            net_resid = slim.batch_norm(net_resid, scope='conv_normal_' + str(layer_no) + '/batch_norm')
            if [conv_st[layer_no], conv_sf[layer_no]]==[1, 1]:
              net = tf.add(net, net_resid, name='conv_normal_' + str(layer_no)+'_res_add')
            else:
              net = net_resid
...
def create_resid_cnn_model(fingerprint_input, model_settings, model_size_info,
                        is_training):
  """Builds a model with depthwise separable convolutional neural network
  Model definition is based on https://arxiv.org/abs/1704.04861 and
  Tensorflow implementation: https://github.com/Zehaos/MobileNet
...
def create_normal_cnn_incr_dropout_model(fingerprint_input, model_settings, model_size_info,
                        is_training):
  """Builds a model with depthwise separable convolutional neural network
  Model definition is based on https://arxiv.org/abs/1704.04861 and
  Tensorflow implementation: https://github.com/Zehaos/MobileNet
      ...
      with slim.arg_scope([slim.batch_norm],
        ...
        for layer_no in range(0, num_layers):
          if layer_no == 0:
            ...
            if is_training:
              net = tf.nn.dropout(net, (layer_no/(num_layers-1))*dropout_prob)
            else:
              net = net
